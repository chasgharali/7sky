...
export const FGEHA_CERT_IMAGE = "/docs/fgeha-building-plan-approval.jpg";
export const FBR_CERT_IMAGE = "/docs/fbr-certificate.jpg";
...
export default function CertificateViewer({
  ...
  const tabsEnabled = showTabs ?? hasPdf;
  const [mode, setMode] = useState<ViewMode>(
    defaultMode === "pdf" && hasPdf ? "pdf" : "image"
  );
      ...
      >
        {tabsEnabled && hasPdf && (
          <div className="flex items-center gap-1 border-b border-white/10 bg-black/20 p-1.5">
            {(
              [
                { id: "image", label: "Certificate Image" },
...
export function FgehaCertificateViewer(props: { className?: string }) {
  return (
    <CertificateViewer
      className={props.className}
      imageSrc={FGEHA_CERT_IMAGE}
      imageAlt="FGEHA Approval of Building Plans for Commercial Plot No 19-A, Markaz G-14, Islamabad"
      title="FGEHA Building Plan Approval — Plot 19-A, G-14 Markaz"
      showTabs={false}
    />
  );
}

export function FbrCertificateViewer(props: { className?: string }) {
  return (
    <CertificateViewer
      className={props.className}
      imageSrc={FBR_CERT_IMAGE}
      imageAlt="FBR Certificate of Confirmation in Respect of Beneficial Owner — One Capital Builders (G334862)"
      title="FBR Beneficial Owner Certificate — One Capital Builders"
      showTabs={false}
    />
  );
}
