...
export default function VerifyOwnershipPage() {
  ...
  const pct = result && result.totalAmount > 0
    ? Math.round((result.amountPaid / result.totalAmount) * 100) : 0;
            ...
            {result.paymentHistorySummary.length > 0 && (
              <div className="bg-[#111] border border-white/10 rounded-2xl overflow-hidden">
                <div className="px-6 py-4 border-b border-white/10">
                  ...
                  <p className="text-gray-500 text-xs mt-0.5">{result.paymentCount} transaction{result.paymentCount !== 1 ? "s" : ""}</p>
                </div>
                <div className="divide-y divide-white/[0.04]">
